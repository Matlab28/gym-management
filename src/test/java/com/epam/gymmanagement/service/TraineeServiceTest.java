...
class TraineeServiceTest {
    ...
    void getTraineeProfileRequiresAccessAndMapsProfile() {
        ...
        TraineeProfileResponseDTO mapped = new TraineeProfileResponseDTO();
        ...
        TraineeProfileResponseDTO response = traineeService.getTraineeProfile("trainee.user");
    ...
    void updateTraineeProfilePersistsUserAndTraineeChanges() {
        ...
        TraineeProfileResponseDTO response = traineeService.updateTraineeProfile("trainee.user", request);
    ...
    void getNotAssignedActiveTrainersFiltersAssignedTrainerAndMapsRemainingTrainers() {
        ...
        verify(securityService).requireSelfOrAdmin("trainee.user", UserRole.TRAINEE);
        verify(gymMapper, never()).toTrainerShortResponse(assignedTrainer);
    }
    ...
    void updateTraineeTrainersReplacesAssignmentsWithActiveTrainers() {
        ...
        TrainerShortResponseDTO firstResponse = new TrainerShortResponseDTO();
        ...
        when(gymMapper.toTrainerShortResponse(secondTrainer)).thenReturn(secondResponse);

        TrainerAssignmentResponseDTO response = traineeService.updateTraineeTrainers("trainee.user", request);

        assertEquals(List.of(firstTrainer, secondTrainer), trainee.getTrainers());
        assertEquals(List.of(firstResponse, secondResponse), response.getTrainers());
        verify(securityService).requireSelfOrAdmin("trainee.user", UserRole.TRAINEE);
    }
    ...
    private UpdateTraineeTrainersRequestDTO trainerAssignmentRequest(String... usernames) {
        List<TrainerUsernameRequestDTO> trainers = List.of(usernames)
                .stream()
                .map(this::trainerUsernameRequest)
                .toList();
    ...
    private TrainerUsernameRequestDTO trainerUsernameRequest(String username) {
        TrainerUsernameRequestDTO request = new TrainerUsernameRequestDTO();
